...
const int n_materials= 8; // idem que pour lights
uniform mat4 materials[n_materials];
...
void main(void) {
	// decode materials
	vec3 mat_ambient= vec3(materials[int(idx_material)][0]);
	vec3 mat_diffuse= vec3(materials[int(idx_material)][1]);
	float mat_shininess= materials[int(idx_material)][2][0];

	vec3 scattered_light= mat_ambient;
	...
	for (int i=0; i<n_lights; i++) {
		...
		// je ne sais pas pourquoi ca foire sinon
		if (i!=0) { continue; }
		...
		if (diffuse== 0.) {
			...
			specular= pow(specular, mat_shininess)* l.strength;
		}
		//attenuation= 0.5;
	...
	vec3 rgb= min(vec3(1.), mat_diffuse* scattered_light+ reflected_light);
	
	frag_color= vec4(rgb, alpha);
	
	//frag_color= vec4(1.0, 1.0, 0.0, 1.0);
	//frag_color= vec4(mat_diffuse* scattered_light, 1.0);
	//frag_color= vec4(reflected_light, 1.0);
	//frag_color= vec4(attenuation, 0.0, 0.0, 1.0);
	//frag_color= vec4(1./ (lights[0].constant_attenuation), 0.0, 0.0, 1.0);
	
	//frag_color= vec4(lights[0].quadratic_attenuation, 0.0, 0.0, 1.0);
