...
void BTree::recursive_merge(BTreeNode * node) {
	if (node->_left) {
		...
		float y_min_left= 1e8f;
		unsigned int idx_min_left= 0;
		/*for (auto triangle : node->_left->_triangles) {
			for (unsigned int i=0; i<3; ++i) {
				if (_pts[triangle->_idx[i]].y< y_min_left) {
					y_min_left= _pts[triangle->_idx[i]].y;
					idx_min_left= triangle->_idx[i];
				}
			}
		}*/
		for (unsigned int i=node->_left->_idx_min; i<node->_left->_idx_max+ 1; ++i) {
			if (_pts[i].y< y_min_left) {
				y_min_left= _pts[i].y;
				idx_min_left= i;
			}
		}
		float y_min_right= 1e8f;
		unsigned int idx_min_right= 0;
		for (unsigned int i=node->_right->_idx_min; i<node->_right->_idx_max+ 1; ++i) {
			if (_pts[i].y< y_min_right) {
				bool inter= false;
				for () {
					if (segment_intersects_segment(glm::vec2 & pt1_begin, glm::vec2 & pt1_end, glm::vec2 & pt2_begin, glm::vec2 & pt2_end, glm::vec2 * result)) {
						inter= true;
						break;
					}
				}
				if (!inter) {
					y_min_right= _pts[i].y;
					idx_min_right= i;
				}
			}
		}
