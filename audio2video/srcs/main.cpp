...
void init() {
	...
	*/
	...
	visu_spectrum= new VisuSpectrum(prog_3d, prog_repere, prog_select, audio);
	visu_simu= new VisuSimu(prog_2d, audio);
	visu_art= new VisuArt(prog_3d_obj, prog_repere, prog_select, audio);

	visu_art->load("../data/configs/visu_simple_1.json");
	//visu_art->randomize();
	...
	// ------------------------------------------------------------------------
	stream= pa_init(idx_device_input, idx_device_output, SAMPLE_RATE, SAMPLES_PER_BUFFER, pa_callback, audio);
}
...
void show_infos() {
	...
	texts.push_back(Text(font_str.str(), glm::vec2(10.0f, float(MAIN_WIN_HEIGHT)- 15.0f), font_scale, font_color));

	font_str.str("");
	font_str << "idx_block=" << audio->_current_sample/ SAMPLES_PER_BUFFER << " / " << audio->_n_samples/ SAMPLES_PER_BUFFER;
	texts.push_back(Text(font_str.str(), glm::vec2(10.0f, float(MAIN_WIN_HEIGHT)- 30.0f), font_scale, font_color));
	...
	texts.push_back(Text(font_str.str(), glm::vec2(10.0f, float(MAIN_WIN_HEIGHT)- 45.0f), font_scale, font_color));
	...
	if (visu_art->_last_idx_signature>= 0) {
		...
		texts.push_back(Text(font_str.str(), glm::vec2(10.0f, float(MAIN_WIN_HEIGHT)- 60.0f), font_scale, font_color));
	}
...
void draw() {
	...
	if (visu_art->_fullscreen) {
		...
		set_subwindow(SPECTRUM_BCK, SPECTRUM_WIN_X, SPECTRUM_WIN_Y, SPECTRUM_WIN_WIDTH, SPECTRUM_WIN_HEIGHT);
		visu_spectrum->draw();

		set_subwindow(WAVE_BCK, WAVE_WIN_X, WAVE_WIN_Y, WAVE_WIN_WIDTH, WAVE_WIN_HEIGHT);
		visu_wave->draw();

		set_subwindow(SIMU_BCK, SIMU_WIN_X, SIMU_WIN_Y, SIMU_WIN_WIDTH, SIMU_WIN_HEIGHT);
		visu_simu->draw();

		set_subwindow(ART_BCK, ART_WIN_X, ART_WIN_Y, ART_WIN_WIDTH, ART_WIN_HEIGHT);
		lights_ubo->draw(visu_art->_view_system->_world2clip);
		visu_art->draw();
	}
