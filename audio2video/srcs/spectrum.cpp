...
VisuSpectrum::VisuSpectrum(GLuint prog_draw_3d, GLuint prog_repere, GLuint prog_select, Audio * audio) : _mouse_down(false), _audio(audio) {
	_gl_spectrum= new GLSpectrum(prog_draw_3d, audio);

	_view_system= new ViewSystem(prog_repere, prog_select, MAIN_WIN_WIDTH, MAIN_WIN_HEIGHT);
...
VisuArt::VisuArt(GLuint prog_draw_3d, GLuint prog_repere, GLuint prog_select, Audio * audio) :
	_audio(audio), _last_idx_signature(-1), _mouse_down(false), _fullscreen(false) {
	...
	vector<string> ch_models= list_files("../data/objs", "xml");
	for (auto & ch_model : ch_models) {
		_static_models.push_back(new StaticModel(ch_model, prog_draw_3d));
	}
}
...
bool VisuArt::key_down(InputState * input_state, SDL_Keycode key) {
	if (key== SDLK_c) {
		...
		_audio->_mode= AUDIO_PLAYBACK;
		_audio->loadfromfile("../data/wav/bcl1.wav");
		return true;
	}
	else if (key== SDLK_r) {
		_audio->record2file("../data/wav/record.wav");
		return true;
	}
