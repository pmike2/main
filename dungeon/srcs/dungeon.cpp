...
void Dungeon::randomize() {
	...
	for (unsigned int i=0; i<N_HALLWAYS; ++i) {
		...
		/*if (j== 0) {
			...
			for (unsigned idx_mesh_2=0; idx_mesh_2<aabbs_rooms.size(); ++idx_mesh_2) {
				...
				if ((segment_intersects_aabb(posf0, posf3, aabbs_rooms[idx_mesh_2])) ||
					(segment_intersects_aabb(posf1, posf2, aabbs_rooms[idx_mesh_2]))) {
					is_inter_room= true;
					break;
				}
			}
	...
	aabbs_rooms.clear();
	for (auto bbox : bboxs_hallways) {
		delete bbox;
	}
	bboxs_hallways.clear();
