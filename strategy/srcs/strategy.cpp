	...
	_add_unit_ok(false), _add_unit_fow_ok(false), _move_unit_ok(false), _attack_unit_ok(false)
{
	...
	_gl_draw_manager->add_texture("fow_texture_array", GL_TEXTURE_2D_ARRAY, 0,
		...
		GL_RED, glm::uvec3(_map->_teams[0]->_fow->_n_cols, _map->_teams[0]->_fow->_n_ligs, _map->_teams.size()), GL_RED, GL_FLOAT);
...
void Strategy::set_ihm() {
	_ihm->get_element("edit", "edit")->set_callback([this](){_config->_edit = true;}, [this](){_config->_edit = false;});
	
	_ihm->get_element("general_info", "show_info")->set_callback([this](){_config->_show_info = true;}, [this](){_config->_show_info = false;});
	_ihm->get_element("general_info", "units_paused")->set_callback(
	...
	_ihm->get_element("unit_action", "attack")->set_callback([this](){_config->_unit_action_mode = ATTACK;});

	_ihm->get_element("edit_mode", "add_element")->set_callback([this](){_config->_edit_mode = ADD_ELEMENT;});
	...
	_ihm->get_element("tree_params", "n_trees")->set_callback([this](){
		...
		_config->_n_trees = slider->_value;
	});

	_ihm->get_element("tree_params", "trees_dispersion")->set_callback([this](){
		GLIHMElement * element = _ihm->get_element("tree_params", "trees_dispersion");
		GLIHMSlider * slider = (GLIHMSlider *)(element);
		_config->_trees_dispersion = slider->_value;
	});

	_ihm->get_element("stone_params", "n_stones")->set_callback([this](){
		GLIHMElement * element = _ihm->get_element("stone_params", "n_stones");
		GLIHMSlider * slider = (GLIHMSlider *)(element);
		_config->_n_stones = slider->_value;
	});

	_ihm->get_element("stone_params", "stones_dispersion")->set_callback([this](){
		GLIHMElement * element = _ihm->get_element("stone_params", "stones_dispersion");
		GLIHMSlider * slider = (GLIHMSlider *)(element);
		_config->_stones_dispersion = slider->_value;
	});
...
void Strategy::update_edit_map() {
	...
	if (_config->_edit_mode == EDIT_ELEVATION) {
		...
		if (_config->_element_type == ELEMENT_TREE) {
			...
			// TODO : faire un autre type de curseur ?
			circle_pts = circle_vertices(pt_2d(_cursor_world_position), 1.0, EDIT_MAP_N_VERTICES_PER_CIRCLE);
		}
	}
	else if (_config->_edit_mode == ERASE) {
...
void Strategy::update_cursor() {
	...
	if (_config->_play_mode == ADD_UNIT) {
		...
		if (_add_unit_ok && _add_unit_fow_ok) {
			...
			add_unit_color = glm::vec4(1.0, 0.0, 0.5, 1.0);
		}
		...
		for (uint i=0; i<segs.size(); ++i) {
			...
			ptr[1] = float(segs[i].y);
			ptr[2] = float(segs[i].z + Z_OFFSET_UNIT);
			ptr[3] = add_unit_color.r;
			ptr[4] = add_unit_color.g;
			ptr[5] = add_unit_color.b;
			ptr[6] = add_unit_color.a;
			ptr += 7;
		}
	}
		...
		glm::vec4 move_unit_color;
		if (_move_unit_ok) {
			move_unit_color = glm::vec4(0.0, 0.5, 0.2, 1.0);
		}
		else {
			move_unit_color = glm::vec4(1.0, 0.0, 0.5, 1.0);
		}
		for (uint i=0; i<4; ++i) {
			ptr[0] = float(l_pts[i].x);
			ptr[1] = float(l_pts[i].y);
...
bool Strategy::mouse_button_down(InputState * input_state, time_point t) {
	...
	if (input_state->_left_mouse) {
		if (_config->_edit) {
			...
			else if (_config->_edit_mode == EDIT_ELEVATION) {
				AABB_2D * aabb = new AABB_2D(pt_2d(_cursor_world_position) - pt_2d(_config->_elevation_radius), 2.0 * pt_2d(_config->_elevation_radius));
				_map->remove_elements_in_aabb(aabb);
				_map->remove_units_in_aabb(aabb);
				delete aabb;

				_map->_elevation->set_alti_disk(pt_2d(_cursor_world_position), _config->_elevation_radius, _config->_elevation_mode, _config->_elevation_factor, _config->_elevation_exponent);
				_map->sync2elevation();
...
bool Strategy::mouse_button_up(InputState * input_state, time_point t) {
	...
	if (_config->_edit) {
		...
		if (_config->_play_mode == SELECT_UNIT) {
