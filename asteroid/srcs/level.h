...
public:
	Event();
	Event(EventType type, uint t, glm::vec2 position, std::string enemy);
	~Event();
	friend std::ostream & operator << (std::ostream & os, const Event & event);


	EventType _type; // type
	uint _t; // temps en ms en absolu à partir du début du niveau auquel déclencher cet événement
...
public:
	...
	uint _current_event_idx; // indice de l'événement courant
