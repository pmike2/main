...
int pa_callback(const void * input, void * output, unsigned long frame_count, const PaStreamCallbackTimeInfo * time_info, PaStreamCallbackFlags status_flags, void * user_data) {
	...
	for (unsigned int i=0; i<frame_count; ++i) {
		...
		for (unsigned int idx_channel=0; idx_channel<cvp->_n_output_channels; ++idx_channel) {
			out[i* cvp->_n_output_channels+ idx_channel]= 0.0f;
		}

		for (unsigned int idx_track=0; idx_track<N_MAX_TRACKS; ++idx_track) {
			if (cvp->_cv_tracks[idx_track]->_playing) {
				...
				key_type key= cvp->_cv_tracks[idx_track]->_info._key;
				unsigned int idx_channel= cvp->get_idx_channel(key);
				if (idx_channel== 0) {
					continue;
				}

				out[i* cvp->_n_output_channels+ idx_channel]= 1.0;
				
				cvp->_cv_tracks[idx_track]->_frame_idx++;
				if (cvp->_cv_tracks[idx_track]->_frame_idx>= N_FRAMES_PULSE) {
					cvp->note_off(idx_track);
				}
			}
		}
	}
