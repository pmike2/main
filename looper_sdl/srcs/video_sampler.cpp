...
VideoSample::VideoSample() : _data(0), _width(0), _height(0) {

}


VideoSample::VideoSample(string video_sample_path) : Sample(video_sample_path) {
	// Format I/O context
	...
	*/
	ret = avformat_open_input(&ctx_format, video_sample_path.c_str(), 0, 0);
	if (ret!= 0) {
	...
	*/
	...
	//av_dump_format(ctx_format, 0, video_sample_path.c_str(), false);
	...
	_data= (unsigned char *)av_malloc(buffer_rgb_size* sizeof(unsigned char)* _n_frames);
	...
	while (av_read_frame(ctx_format, pkt)>= 0){
		...
		if (pkt->stream_index == stream_idx) {
			...
			while (ret>= 0) {
				...
				memcpy(_data+ (ctx_codec->frame_number- 1)* buffer_rgb_size* sizeof(unsigned char), buffer_rgb, buffer_rgb_size* sizeof(unsigned char));
			}
