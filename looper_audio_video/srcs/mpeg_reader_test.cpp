...
void init_program(std::string json_path) {
	...
	mpeg_readers = new MPEGReaders(base_index, movie_loc, alpha_loc, movie_time_loc, index_time_loc,
		...
		mpeg_readers->load_json(json_path);
	
	std::cout << mpeg_readers->_config << "\n";
...
void init_mpeg_writer() {
	std::string save_mpeg_path= "../data/sessions/session_"+ current_date_time()+ ".mp4";
